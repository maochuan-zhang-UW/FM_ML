...
windowstring = {'s', 'l'};
dt = 1/200; % Sampling interval (Hz)
P.a.window = [-0.25 1]; % Window based on P picks
...
for gp = 1:length(groups)
    ...
    for kz =3% 1:length(fields)
        ...
        % Save results
        save([path2, 'B_CC/B_', groups{gp}, '_', fields{kz}, '.mat'], 'filteredResultsMatrix','resultsMatrix_cb');
