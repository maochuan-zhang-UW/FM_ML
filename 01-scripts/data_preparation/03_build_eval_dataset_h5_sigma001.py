...
Identical to 02_build_eval_dataset_h5.py except the time-shift sigma is
reduced from 0.02 s to 0.01 s (1 sample at 100 Hz).
...
Run from any directory:
    python /path/to/01-scripts/data_preparation/03_build_eval_dataset_h5_sigma001.py
"""
...
OUT_DIR   = os.path.join(ROOT, "02-data", "K_aug", "TMSF_Val_003")
...
# Time-shift parameters  ← sigma reduced to 0.01 s (1 sample at 100 Hz)
SIGMA_SEC     = 0.01
SIGMA_SAMPLES = SIGMA_SEC * FS          # 1 sample
MAX_SHIFT_SEC = 0.08
...
def main():
    ...
    print("=" * 60)
    print("03_build_eval_dataset_h5_sigma001.py")
    print("=" * 60)
    print(f"Source waveforms : {VAL_H5}")
    print(f"Source noise     : {NOISE_H5}")
    print(f"Output           : {OUT_H5}")
    print(f"Time-shift sigma : {SIGMA_SEC} s  ({SIGMA_SAMPLES} samples at {FS} Hz)")
    ...
    with h5py.File(VAL_H5, "r") as f_in, h5py.File(OUT_H5, "w") as f_out:
        ...
        f_out.attrs["sigma_sec"]          = SIGMA_SEC
